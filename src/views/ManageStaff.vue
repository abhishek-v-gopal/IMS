<template>
  ...
  export default {
    ...
    async created() {
      ...
      this.staffMembers = staffSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
    },
    computed: {
      totalStaff() {
    ...
    methods: {
      ...
      toggleStaffStatus(staff) {
        ...
        if (index !== -1) {
          this.staffMembers[index].status = staff.status === 'active' ? 'inactive' : 'active'
          this.showNotification('success', `${staff.name} is now ${this.staffMembers[index].status}`)
        }
      },
      
      promoteToManager(staff) {
        // In a real app, you would make an API call to promote the user
        const index = this.staffMembers.findIndex(s => s.id === staff.id)
        if (index !== -1) {
          this.staffMembers[index].role = 'manager'
          this.staffMembers[index].priority = false
          this.staffMembers[index].permissions = [
            'Manage Components',
            'Approve Requests',
            'Reject Requests',
            'View Reports'
      ...
      async submitStaffForm() {
        try {
          if (this.showEditStaffModal) {
            ...
            await addDoc(collection(db, "users"), this.staffForm);
          }
          this.showNotification("success", "Staff member saved successfully!");
          this.cancelStaffForm();
        } catch (error) {
          this.showNotification("error", "Failed to save staff member.");
        }
      },
